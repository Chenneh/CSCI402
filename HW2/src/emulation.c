...
void serveOne(int serverNum) {
    ...
    // stat
    pthread_mutex_lock(&m);
    totalQ1Time += (packetMoved->Q1OutTime - packetMoved->Q1InTime);
    totalQ2Time += (packetMoved->Q2OutTime - packetMoved->Q2InTime);
...
void showParameters() {
    ...
    if (DETERMINISTIC)
    {
        fprintf(stdout, "\tlambda = %.8g\n", lambda);
        fprintf(stdout, "\tmu = %.8g\n", mu);
        fprintf(stdout, "\tr = %.8g\n", r);
        fprintf(stdout, "\tB = %i\n", B);
        fprintf(stdout, "\tP = %i\n", P);
    } else {
...
void showStat() {
    ...
    if (packetArrived == 0) {
        ...
        fprintf(stdout, "\taverage packet inter-arrival time time = %.8g\n", averageInterArrivalTime / 1000.0);
    }
    if (packetCompleted == 0) {
        fprintf(stdout, "\taverage packet service time = N/A, no packet was served\n");
    } else {
        fprintf(stdout, "\taverage packet service time = %.8g\n", averageServiceTime / 1000.0);
    }
    fprintf(stdout, "\n");
    
    fprintf(stdout, "\taverage number of packets in Q1 = %.8g\n", totalQ1Time / totalEmulationTime);
    fprintf(stdout, "\taverage number of packets in Q2 = %.8g\n", totalQ2Time / totalEmulationTime);
    fprintf(stdout, "\taverage number of packets in S1 = %.8g\n", totalS1Time / totalEmulationTime);
    fprintf(stdout, "\taverage number of packets in S2 = %.8g\n", totalS2Time / totalEmulationTime);
    ...
    if (packetCompleted == 0) { 
        ...
        fprintf(stdout, "\taverage time a packet spent in system = %.8g\n", averageInSystemTime / 1000.0);
        fprintf(stdout, "\tstandard deviation for time spent in system = %.8g\n", stdDevSystemTime / 1000.0);
    }
    fprintf(stdout, "\n");
    ...
    if (tokenProduced == 0) {
        ...
        fprintf(stdout, "\ttoken drop probability = %.8g\n", tokenDroppedProb);
    }
    ...
    if (packetArrived == 0) {
        ...
        fprintf(stdout, "\tpacket drop probability = %.8g\n", packetDroppedProb);
    }
    fprintf(stdout, "\n");
